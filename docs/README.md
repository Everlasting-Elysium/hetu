...
├── wallpaper-api.md   wallpaper 插件 API（筛选/排序/随机/每日/合集/下载/zip）
...
| [wallpaper-api.md](./wallpaper-api.md) | wallpaper 插件 API：公开匿名只读的壁纸画廊——筛选/排序/随机/每日一图/合集浏览、强制下载与 zip 打包；零迁移，默认不启用（`HETU_PLUGINS=dam,nas,wallpaper` 开启）。 |
